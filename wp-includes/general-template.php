...
function paginate_links( $args = '' ) {
	...
	if ( $args['prev_next'] && $current && 1 < $current ) :
		...
		$page_links[] = sprintf(
			'<li><a class="prev page-numbers" href="%s">%s</a></li>',
			/**
	...
	for ( $n = 1; $n <= $total; $n++ ) :
		if ( $n == $current ) :
			$page_links[] = sprintf(
				'<li class="active"><a href="#!"><span aria-current="%s" class="page-numbers current">%s</span></a></li>',
			...
			if ( $args['show_all'] || ( $n <= $end_size || ( $current && $n >= $current - $mid_size && $n <= $current + $mid_size ) || $n > $total - $end_size ) ) :
				...
				$page_links[] = sprintf(
					'<li><a class="page-numbers" href="%s">%s</a></li>',
					/** This filter is documented in wp-includes/general-template.php */
					esc_url( apply_filters( 'paginate_links', $link ) ),
					$args['before_page_number'] . number_format_i18n( $n ) . $args['after_page_number']
				);
			...
			elseif ( $dots && ! $args['show_all'] ) :
				$page_links[] = '<li class="active"><a href="#!"><span class="page-numbers dots">' . __( '&hellip;' ) . '</span></a></li>';
	...
	if ( $args['prev_next'] && $current && $current < $total ) :
		...
		$page_links[] = sprintf(
			'<li><a class="next page-numbers" href="%s">%s</a></li>',
			/** This filter is documented in wp-includes/general-template.php */
			esc_url( apply_filters( 'paginate_links', $link ) ),
			$args['next_text']
		);
